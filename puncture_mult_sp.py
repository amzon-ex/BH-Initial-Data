...
import sys
from numpy import zeros, size, sqrt, linspace, vstack
...
class EllipticSolver:
    ...
    def solve(self):
        ...
        sol_1d = spsolve(spA, self.rhs_1d, permc_spec = 'MMD_AT_PLUS_A', use_umfpack = True)
...
class Puncture:
        ...
        for i in range(0, n_grid):
            for j in range(0, n_grid):
                for k in range(0, n_grid):
                    ...
                    fac = 3.0 / (2.0 * s2)
                    A_xx = nsum(fac * (2.0 * p_x * l_x - (1.0 - l_x * l_x) * lP))
                    A_yy = nsum(fac * (2.0 * p_y * l_y - (1.0 - l_y * l_y) * lP))
                    A_zz = nsum(fac * (2.0 * p_z * l_z - (1.0 - l_z * l_z) * lP))
                    A_xy = nsum(fac * (p_x * l_y + p_y * l_x + l_x * l_y * lP))
                    A_xz = nsum(fac * (p_x * l_z + p_z * l_x + l_x * l_z * lP))
                    A_yz = nsum(fac * (p_y * l_z + p_z * l_y + l_y * l_z * lP))
                    ...
                    self.alpha[i, j, k] = 2.0 / nsum(1 / s_bh)
    ...
    def write_to_file(self):
        ...
        if out:
            k = self.n_grid // 2
            out.write(
                f"# Data for black hole at x = ({self.bh_loc[0]}, {self.bh_loc[1]}, {self.bh_loc[2]})\n")
            out.write(f"# with linear momentum P = ({self.lin_mom[0]}, {self.lin_mom[1]}, {self.lin_mom[2]})\n")
...
def main():
    ...
    bh_loc = vstack(( loc_x, loc_y, loc_z ))
    # linear momentum
    lin_mom = vstack(( p_x, p_y, p_z ))
    #
